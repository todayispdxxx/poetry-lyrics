...
const width = 800;
const height = 300;
const margin = {top: 130, right: 50, bottom: 20, left: 70}; 
const dotRadius = 1.5;  
const decorationRadius = 3.5;   
const decorationPadding = 10;   

// 创建SVG容器 - 更新选择器
const svg = d3.select("#chart1")  // 修改这里以匹配新的ID
  .append("svg")
    .attr("width", width)
    .attr("height", height)
  .append("g")
...
d3.json("https://raw.githubusercontent.com/todayispdxxx/poetry-lyrics/refs/heads/main/DATA/matches.json")
  .then(data => {
    ...
    const songData = data.find(d => d.actual_song === "中国喜事" && d.actual_singer === "凤凰传奇");
    if (!songData) {
